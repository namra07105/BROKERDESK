...
if($hasFilter)
{
    ...
    if($type != '')
    {
        $typeSafe = mysqli_real_escape_string($con, $type);
        $conditions[] = "type='$typeSafe'";
    }


    if($stype != '')
    {
        $stypeSafe = mysqli_real_escape_string($con, $stype);
        $conditions[] = "stype='$stypeSafe'";
    }


    if($city != '')
    {
        $citySafe = mysqli_real_escape_string($con, $city);
        $conditions[] = "city='$citySafe'";
    }


    if($state != '')
    {
        $stateSafe = mysqli_real_escape_string($con, $state);
        $conditions[] = "state='$stateSafe'";
    }
...
if($result && mysqli_num_rows($result) > 0)
{
    ...
    while($row = mysqli_fetch_array($result))
    {
        ...
        $propertyImage = !empty($row['18'])
            ? "admin/property/" . htmlspecialchars($row['18'])
            : "images/propertyimage.jpg";
...
<a
    href="propertydetail.php?pid=<?php echo urlencode($row['0']); ?>"
>
...
<span>
    ...
    <?php echo htmlspecialchars($row['14']); ?>
...
window.addEventListener("scroll", function () {
    ...
    const moveY = -(maxMove * percentage);
